        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class=" bfr">
                          <a class="btn btn-info" onclick="opadd()" >新增</a>
                             ...
                             <!-- <tbody>
                                <tr class="gradeX">
                                   ...
                                   <td> 
                                     ...
                                     <a onclick="deleteroww(this)" class="btn btn-primary  btn-xs ">删除
                                   </td>
 ...
 <script>
   var table;
		...
		//		"bStateSave":true,
				...
				"dom" : 'Blfrtip',
				"ajax" : {
					"url" : "${path}/dictController/datagrid",
					"type" :"post",
					"data" : function(d) {
				...
				"columnDefs" : [
						{
							...
							"render" : function(data, type, row) {
								var buttons = '';
									buttons +="<a href='javascript:void(0);' class='btn btn-primary  btn-xs' onClick=\"dict_ed('" + row.id + "')\" >编辑</a>";
				                    buttons +=" <a href='javascript:void(0);' class='btn btn-primary  btn-xs ' onClick=\"dict_del('" + row.id + "')\" >删除</a>";
								return buttons;
							},
							"targets" :5,
						}],
        ...
        function dict_del(id) {
            layer.confirm('确认删除？', {
                              btn: ['确认','取消'] //按钮
                              	 ...
                              	 $.ajax({
                       				type: "post",
                       				url : '${path}/dictController/deleteElement',
                       				data:{
                       					id:id
                       				},
                       				dataType:'json',
    ...
    //打开编辑页面
        ...
        function dict_ed(id) {
            layer.open({
                ...
                ,maxmin: false //允许全屏最小化
                ,anim: 5 //0-6的动画形式，-1不开启
                ,content: ['${path}/dictController/toEditPage?id='+id]
            	  ,end: function(){
            			//layer.msg("修改成功");
  		    		//table.fnDraw();
  		    		table._fnAjaxUpdate(table.oSettings);
