            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class=" bfr">
                            <a class="btn btn-info" onclick="opadd()" >新增菜单</a>
                        ...
                        <table class="table table-striped table-bordered table-hover text-center" id="editable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="col-sm-1">上级菜单</th>
                                    <th class="col-sm-2">菜单路径</th>
                                    <th class="col-sm-1">菜单颜色</th>
                                    <th class="col-sm-1">菜单图标</th>
                                    <th class="col-sm-1">首页路径</th>
 ...
 -->
    ...
    var table;
		...
		//		"bStateSave":true,
				...
				"dom" : 'Blfrtip',
				"ajax" : {
					"url" : "${path}/functionController/datagrid",
					"type" :"post",
					"data" : function(d) {
						//keywords search params pass to server
						var searchtext = $(".dataTables_wrapper .dataTables_filter input").val();
				...
				"columnDefs" : [
						{
							...
							"render" : function(data, type, row) {
								var buttons = '';
									buttons +="<a href='javascript:void(0);' class='btn btn-primary  btn-xs' onClick=\"function_ed('" + row.id + "')\" >编辑</a>";
				                    buttons +=" <a href='javascript:void(0);' class='btn btn-primary  btn-xs ' onClick=\"function_del('" + row.id + "')\" >删除</a>";
								return buttons;
							},
							"targets" :8,
						}],
        ...
        function function_del(id) {
            layer.confirm('确认删除？', {
                              btn: ['确认','取消'] //按钮
                              	 ...
                              	 $.ajax({
                       				type: "post",
                       				url : '${path}/functionController/deleteElement',
                       				data:{
                       					id:id
                       				},
                       				dataType:'json',
    ...
    //打开编辑页面
       /*  function opbj(n) {
          layer.open({
              ...
              ,maxmin: false //允许全屏最小化
              ,anim: 5 //0-6的动画形式，-1不开启
              ,content: ['editcdgl.jsp']
            });    
  
        }; */
 	 function function_ed(id) {
            layer.open({
                type: 2 //Page层类型
                ,area: ['100%', '100%']
                ,title: '编辑'
                ...
                ,maxmin: false //允许全屏最小化
                ,anim: 5 //0-6的动画形式，-1不开启
                ,content: ['${path}/functionController/toEditPage?id='+id]
            	  ,end: function(){
            			//layer.msg("修改成功");
  		    		//table.fnDraw();
  		    		table._fnAjaxUpdate(table.oSettings);
       ...
       /*  function opadd(n) {
		...
		function opadd() {
            layer.open({
                ...
                ,maxmin: false //允许全屏最小化
                ,anim: 5 //0-6的动画形式，-1不开启
                ,content: ['${path}/functionController/getPids']
            	  ,end: function(){
            		table._fnAjaxUpdate(table.oSettings);
  	    	}
